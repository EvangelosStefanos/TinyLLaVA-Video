...
LLM_VERSION=/data/vlm/zxj/checkpoints/phi-2 # llm path
VT_VERSION=/data/vlm/zxj/checkpoints/siglip-so400m-patch14-384 #vision tower path
VT_VERSION2="" #if you are not using mof vision tower, keep it empty
CN_VERSION=resamplerwithpe #connector type
CONV_VERSION=phi #chat template
...
bash scripts/train/pretrain_video.sh "$VIDEO_DATA_PATH" "$VIDEO_PATH" "$LLM_VERSION" "$VT_VERSION" "$VT_VERSION2" "$CN_VERSION" "$VERSION" "$TRAIN_RECIPE" "$MODEL_MAX_LENGTH" "$NUM_FRAME" "$NUM_QUERY" "$GROUP"
bash scripts/train/finetune_video.sh "$FINETUNE_VIDEO_DATA_PATH" "$FINETUNE_VIDEO_PATH" "$LLM_VERSION" "$VT_VERSION" "$VT_VERSION2" "$CN_VERSION" "$CONV_VERSION" "$VERSION" "$TRAIN_RECIPE" "$MODEL_MAX_LENGTH" "$NUM_FRAME" "$NUM_QUERY" "$GROUP"
