...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x, masks=None):
        """
        :param x: input image/video tensor of shape [B, C, H, W] or [B, C, T, H, W]
        :param masks: indices of patch tokens to mask (remove)
        :returns: tensor of shape [B, N, D] where N is the number of unmasked tokens
        num_patches = (T / tubelet_size) * (H / patch_size) * (W / patch_size)
        N = (1 - mask_ratio) * num_patches
        D = embed_dim
        """
